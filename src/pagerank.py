	...
	for i in range(N):
		for j in range(N):
			...
			if contexts_sim[i][j] > 0.5:
				wt = 1 - contexts_sim[i][j]
				G.add_edge(i, j, weight=wt)
				G.add_edge(j, i, weight=wt)


def add_edges_between_questions(questions_sim, G, M, start, end):
...
def main(args):
	...
	print len(contexts_sim)
	print len(contexts_sim[0])
	...
	add_edges_between_questions(questions_sim, G, M, start=0, end=10000)
	questions_sim = None
